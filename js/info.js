const parametersTimeCalc = {
  "МА-20": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {},
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МА-25": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МА-30": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МА-50": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 187,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 42,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 910,
      paramSpeed: 40,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 910,
      paramSpeed: 40,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МА-100": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 187,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 42,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 560,
      paramSpeed: 35,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 560,
      paramSpeed: 35,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МА-200": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 187,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 42,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МТЛ-38": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МК-32": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МК-43": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  МК48: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "МК многослойная": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ЛМА-9": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ЛМАС: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ММ-35": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ЛКС0925: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  Бумажная: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ЛКБ: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ЛКА: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  П600: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  П1000: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ПЭ с релизом": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ПЭ с лайнером": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  Газетная: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ПЭТ 12 мкм": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ПСТ2023 1050": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ПСТ2023 1240": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ПСТ с печатью": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ЛПА серая": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ЛПА черная": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ЛПА белая": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ЛПА синяя": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "ЛПА красная": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ВК3: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ВК6: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ВК9: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ВК19: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ВК32: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ВК40: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ВК50: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППЭ1: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППЭ2: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППЭ3: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППЭ4: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППЭ5: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППЭ8: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППЭ10: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППЭ20: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  Войлочная: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  Картон: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  Кромка: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  СТ: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ФТ: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  СВЭМП: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "Лента 8500 (ЛПС)": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППУ5: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  ППУ10: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  МА30Б70: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2БОПП": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2БП": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2НТ": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2ПЭТ": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  НТПЭТ1050: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  НТПЭТ1250: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2ППЭ1": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2ППЭ2": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2ППЭ3": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2ППЭ5": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  МКА: {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
  "2ЛКА": {
    param1: {
      paramName: "1 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 187,
    },
    param2: {
      paramName: "1 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 187,
    },
    param3: {
      paramName: "1 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param4: {
      paramName: "1 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param5: {
      paramName: "1 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param6: {
      paramName: "1 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param7: {
      paramName: "1 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param8: {
      paramName: "1 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 854,
    },
    param9: {
      paramName: "1 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 874,
    },
    param10: {
      paramName: "1 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 180,
    },
    param11: {
      paramName: "1 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param12: {
      paramName: "1 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param13: {
      paramName:
        "1 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param14: {
      paramName:
        "1 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param15: {
      paramName: "1 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param16: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "1 сторона - Полив",
      paramValue: () => {
        console.log(paramWidth.value);
      },
    },
    param17: {
      paramName: "2 сторона - Зарядка материалов - Вал 1 - 1 джамбо материала",
      paramValue: 0,
    },
    param18: {
      paramName: "2 сторона - Зарядка материалов - Вал 2 - 1 джамбо материала",
      paramValue: 0,
    },
    param19: {
      paramName: "2 сторона - Зарядка материалов - Вал 3 - 1 джамбо материала",
      paramValue: 0,
    },
    param20: {
      paramName: "2 сторона - Буферный лайнер - Монтаж",
      paramValue: 0,
    },
    param21: {
      paramName: "2 сторона - Буферный лайнер - Снятие",
      paramValue: 0,
    },
    param22: {
      paramName: "2 сторона - Настр..материал - Монатж - Заказ-наряд",
      paramValue: 0,
    },
    param23: {
      paramName: "2 сторона - Настр..материал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param24: {
      paramName: "2 сторона - Настройка - Предварительная - Заказ-наряд",
      paramValue: 0,
    },
    param25: {
      paramName: "2 сторона - Настройка - Окончательная - Заказ-наряд",
      paramValue: 0,
    },
    param26: {
      paramName: "2 сторона - Заполнение - заказ-наряда - Заказ-наряд",
      paramValue: 0,
    },
    param27: {
      paramName: "2 сторона - Банан-вал - Монтаж - Заказ-наряд",
      paramValue: 0,
    },
    param28: {
      paramName: "2 сторона - Банан-вал - Снятие - Заказ-наряд",
      paramValue: 0,
    },
    param29: {
      paramName:
        "2 сторона - Операции к джамбо - Операции до полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param30: {
      paramName:
        "2 сторона - Операции к джамбо - Операции после полива - 1 джамбо полуфабриката",
      paramValue: 0,
    },
    param31: {
      paramName: "2 сторона - Упаковка - 1 джамбо полуфабриката",
      paramValue: () => {},
    },
    param32: {
      paramWidth: 1050,
      paramWinding: 1520,
      paramSpeed: 50,
      paramName: "2 сторона - Полив",
      paramValue: () => {},
    },
  },
};

// function calcNeedWidth(width) {
//   return needWidthList[width];
// }
// export function addCalcNeedWidth() {
//   for (let i = 1; i < tableNotes.length; i++) {
//     let needWidth = calcNeedWidth(tableNotes[i].titleTypeOfTape);
//     console.log(calcNeedWidth(tableNotes[i].titleTypeOfTape));
//     tableNotes[i].titleAmount = needWidth;
//   }
// }
// addCalcNeedWidth();
// console.log(calcNeedWidth(tableNotes[1].titleTypeOfTape));
// console.log(tableNotes[1].titleTypeOfTape);
